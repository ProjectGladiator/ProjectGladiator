...
namespace DataProtocol {
	...
	enum PGAMEDATA {
		...
		PGAMEDATA_PARTY_DUNGEON_STAGE_SPAWNINFO,		// 파티 스테이지 입장시 좌표 - (코드,좌표)
		PGAMEDATA_STAGE_MONSTER_TPYES_COUNT,			// 스테이지 몬스터 종류가 몇개인지 - (종류 숫자)
		PGAMEDATA_STAGE_MONSTER_INFO					// 스테이지 몬스터 정보 - (몬스터코드,몬스터숫자,좌표)
	};
	// 공통 프로토콜
	enum PGLOBAL {
		CONFIRM = 0
